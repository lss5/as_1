<?php

return [

    /*
    |--------------------------------------------------------------------------
    | Notifications/Emails Language Lines
    |--------------------------------------------------------------------------
    */

    'product' => [
        'status' => [
            'created' => [
                'title' => 'Listing created!',
                'subject' => 'Listing is created and on moderates!',
                'body' => 'Your product has been created. The product will be available on the site after moderation.'
            ],
            'active' => [
                'title' => 'Listing activated!',
                'subject' => 'Listing activated',
                'body' => 'Your listing has successfully passed moderation and is now available on the site!'
            ],
            'moderated' => [
                'title' => 'Listing moderated!',
                'subject' => 'Listing available to publish',
                'body' => 'Your product has been sent for moderation. The product will be available on the site after moderation.'
            ],
            'moderation' => [
                'title' => 'Listing sent for moderation',
                'subject' => 'Listing sent for moderation',
                'body' => 'Your product has been sent for moderation. The product will be available on the site after moderation.'
            ],
            'expired' => [
                'title' => 'Listing has been Expired',
                'subject' => 'Listing Expired',
                'body' => 'Your product has been Expired'
            ],
            'banned' => [
                'title' => 'Listing Banned!',
                'subject' => 'Listing Banned',
                'body' => 'Your product has been Banned'
            ],
            'canceled' => [
                'title' => 'Listing Canceled',
                'subject' => 'Listing Canceled',
                'body' => 'Sorry, but your listing not check rules for Activation'
            ],
            'restored' => [
                'title' => 'Listing Restored',
                'subject' => 'Listing Restored',
                'body' => 'Your listing restored and will been available after moderation'
            ],
        ],
    ],
    
];
