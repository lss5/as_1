...
{{-- 
        ...
        <table class="table w-100">
            <tbody>
                    ...
                    @foreach ($user->contacts as $contact)
                        <tr>
                            <td colspan="2" class="d-flex justify-content-start align-items-center flex-wrap">
                                ...
                                <form action="{{ route('profile.contacts.setmain', $contact) }}" method="POST">
                                    @method('PATCH')
                                    @csrf
                                ...
                                <form action="{{ route('profile.contacts.destroy', $contact) }}" method="POST">
                                    @method('DELETE')
                                    @csrf
                ...
                <tr>
                    <td colspan="2">
                        <form action="{{ route('profile.contacts.store', $user) }}" method="POST" class="row">
                            @csrf
            ...
            <tbody>
                <tr>
                    ...
                    <td>
                        ...
                        @else
                            Off
                            <a href="{{ route('profile.f2a', $user) }}" type="button" class="btn btn-outline-success btn-sm mx-2" title="Switch to Authentication App (TOTP) now">Enable</a>
                        @endif
                    </td>
                </tr>
                <tr>
                    <td colspan="2">
                        <small>As we are constantly improving privacy, security and integrity, AsicOffer establishes TOTP - Time-based One-Time Password providing the option to use one-time passwords generated by app, like Google Authenticator to protect your account.</small>
                    </td>
                </tr>
            </tbody>
        </table>
...
<div class="row">
    <div class="col-12">
        <a href="{{ route('profile.edit') }}" type="button" class="btn btn-outline-secondary btn-sm mx-1">{{ __('product.btn.edit') }}</a>
    </div>
</div>
@endsection
