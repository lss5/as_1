...
@section('content')
    <div class="content-wrapper">
        ...
        <section class="content">
            <form method="POST" action="{{ route('profile.companies.update', $company) }}" class="w-100" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <div class="form-group">
                                    <img class="profile-user-img border-1" src="{{ asset('storage/'.$company->images()->where('mark', 'logo')->first()->link) }}">
                                ...
                                <div class="form-group">
                                    <img class="profile-user-img border-1" src="{{ asset('storage/'.$company->images()->where('mark', 'header')->first()->link) }}">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" id="name" name="name" value="{{ old('name') ?? $company->name }}" class="form-control @error('name') is-invalid @enderror">
                                </div>
                                <div class="form-group">
                                    <label>Description</label>
                                    <textarea class="form-control" rows="4" name="description" placeholder="About your company">{{ old('description') ?? $company->description }}</textarea>
                                </div>
                                <div class="form-group">
                                    <label for="inputLegalAddress">Legal Address</label>
                                    <input type="text" id="inputLegalAddress" name="legal_address" value="{{ old('legal_address') ?? $company->legal_address }}" class="form-control @error('legal_address') is-invalid @enderror">
                                </div>
                                <div class="form-group">
                                    <label for="inputActualAddress">Actual Address</label>
                                    <input type="text" id="inputActualAddress" name="actual_address" value="{{ old('actual_address') ?? $company->actual_address }}" class="form-control @error('actual_address') is-invalid @enderror">
                                </div>
                                <div class="form-group">
                                    ...
                                    <select class="custom-select rounded-0 @error('country') is-invalid @enderror" name="country" id="selectContry">
                                        ...
                                        @foreach ($countries as $country)
                                            <option value="{{ $country->id }}" @if(old('country') == $country->id || $company->country_id == $country->id) selected @endif>{{ $country->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
